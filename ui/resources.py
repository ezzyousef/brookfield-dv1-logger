"""Locates bundled assets (icons, EML logo) whether running from source or
from a PyInstaller-frozen executable.

PyInstaller onefile builds unpack data files into a temporary folder at
`sys._MEIPASS` at runtime; running from source, the project root is just
this file's grandparent directory. Both cases resolve through the same
function so callers never need an if/else for "am I frozen."
"""
import sys
...
# All PNG sizes generated for assets/app_icon_*.png -- kept as one explicit
...
def load_app_icon():
    ...
    that produces a blank/generic taskbar icon for a frozen Python app.
